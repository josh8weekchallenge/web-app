runWhenMemberstackReady(async function (memberstack) {
  ...
  // Add these test functions to help debug - you can call them from browser console
  
  // Test 1: Check if remove buttons exist in HTML
  function testRemoveButtonsExist() {
    console.log("🧪 TEST: Checking if remove buttons exist in HTML");
    
    const allRemoveBtns = document.querySelectorAll('[data-remove-extra-workout]');
    console.log(`Found ${allRemoveBtns.length} remove buttons in DOM`);
    ...
    if (allRemoveBtns.length === 0) {
      console.error("❌ NO REMOVE BUTTONS FOUND! You need to add them to your HTML.");
      console.log("Add this to each workout card:");
      console.log(`<button data-remove-extra-workout="" style="display: none;">Remove</button>`);
    }
  }
  
  // Test 2: Check DOM cache
  function testDOMCache() {
    console.log("🧪 TEST: Checking DOM cache");
    
    DOM.workoutCards.forEach((cache, index) => {
      console.log(`Card ${index}:`, {
        hasElement: !!cache.element,
        hasRemoveBtn: !!cache.removeBtn,
        removeBtnVisible: cache.removeBtn?.style.display,
        workoutSlug: cache.element?.getAttribute('data-workout-slug')
      });
    });
  }
  
  // Test 3: Simulate button click
  function testButtonClick(workoutSlug) {
    console.log(`🧪 TEST: Simulating click on remove button for ${workoutSlug}`);
    
    const btn = document.querySelector(`[data-remove-extra-workout="${workoutSlug}"]`);
    if (btn) {
      console.log("Found button, simulating click...");
      btn.click();
    } else {
      console.error(`No button found with data-remove-extra-workout="${workoutSlug}"`);
    }
  }
  ...
  function testEventListeners() {
    ...
    // Create a test button and see if click handler works
    const testBtn = document.createElement('button');
    testBtn.setAttribute('data-remove-extra-workout', 'test-workout-slug');
    testBtn.textContent = 'Test Remove Button';
    testBtn.style.cssText = 'position: fixed; top: 10px; left: 10px; z-index: 9999; background: red; color: white; padding: 10px;';
    
    document.body.appendChild(testBtn);
    
    console.log("Added test button to page. Click it to test event listener.");
  ...
  // Test 5: Check current workouts
  function testCurrentWorkouts() {
    console.log("🧪 TEST: Current workouts analysis");
    
    console.log(`Total workouts: ${currentWorkouts.length}`);
    
    currentWorkouts.forEach((workout, i) => {
      const isExtra = workout.slug?.includes('extra-workout');
      console.log(`Workout ${i + 1}: ${workout.name}`, {
        slug: workout.slug,
        isExtra: isExtra,
        cardIndex: i + 1
      });
      ...
      if (isExtra) {
        const expectedBtn = document.querySelector(`[data-remove-extra-workout="${workout.slug}"]`);
        console.log(`  Remove button exists: ${!!expectedBtn}`);
        if (expectedBtn) {
          console.log(`  Button visible: ${expectedBtn.style.display !== 'none'}`);
        }
      }
    });
  }
  
  // Call this after page loads to run all tests
  function runAllTests() {
    console.log("🚀 RUNNING ALL DEBUG TESTS");
    console.log("=" * 50);
    
    testRemoveButtonsExist();
    console.log("-" * 30);
